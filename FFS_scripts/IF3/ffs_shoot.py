...
input = 'input'
logfilename = 'ffs.log'
starting_conf_pattern = '../IF2/success*'
...
# interface lambda_{n}
lambda_n_name = 'bonds_01'
lambda_n_value = TOEHOLD

# interface lambda_{n+1}
lambda_m_name = 'bonds_01'
lambda_m_value = TOEHOLD + 3
lambda_m_compar = '>='
...
#condition_file.write("condition2 = {\n%s %s %s\n %s %s %s \n}\n" % (lambda_m_name, lambda_m_compar, str(lambda_m_value),  lambda_b_name, lambda_b_compar, str(lambda_b_value)))
...
def f(idx):
	...
	while success_count.value < desired_success_count:
		...
		#log ("Worker %d started" % idx)
		conf_index = rnd.choice (range(0, len(starting_confs))) # maybe we should start from 0
