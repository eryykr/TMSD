...
'''
TOEHOLD = 4
BMD = 11 #(branch migration domain)
####################################
# edit here
####################################
# number of successes, can be changed via command line
desired_success_count = 1000
...
# interfaces 
# interface lambda_{-1}
lambda_f_name = 'dist_01'
lambda_f_value = 4.

# interface lambda_{0}
lambda_0_name = 'dist_01'
lambda_0_compar = '<='
lambda_0_value = 1.

# interface lambda_{success}
lambda_s_name = 'bonds_01'
lambda_s_compar = '>='
lambda_s_value = BMD  + TOEHOLD

lambda_s2_name = 'dist_02'
lambda_s2_value = 4
lambda_s2_compar = '>'
####################################
...
#additional condition of the incumbent fully unbinding
condition_file.write("condition2 = {\n%s %s %s\n %s %s %s \n}\n" % (lambda_s_name, lambda_s_compar, str(lambda_s_value),  lambda_s2_name, lambda_s2_compar, str(lambda_s2_value)))
condition_file.close()
...
def f(lidx):
	...
	while success_count.value < desired_success_count:
		...
		command = my_base_command + ['seed=%d' % myrng.randint(1,50000), 'ffs_file=never.txt', 'steps=1e5', 'refresh_vel=1']
